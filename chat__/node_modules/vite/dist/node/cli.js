...
require('fs');
require('path');
var build = require('./chunks/dep-d7654bd1.js');
require('tty');
require('util');
require('net');
...
cli
    ...
    // is ok here
    const { createServer } = await Promise.resolve().then(function () { return require('./chunks/dep-d7654bd1.js'); }).then(function (n) { return n.index$1; });
    try {
        ...
        const logError = build.createLogger(options.logLevel).error;
        logError(build.source.red(`error when starting dev server:\n${e.stack}`));
        process.exit(1);
    }
});
// build
cli
    ...
    .action(async (root, options) => {
    const { build: build$1 } = await Promise.resolve().then(function () { return require('./chunks/dep-d7654bd1.js'); }).then(function (n) { return n.build$1; });
    try {
        await build$1({
            root,
            mode: options.mode,
        ...
        const logError = build.createLogger(options.logLevel).error;
        logError(build.source.red(`error during build:\n${e.stack}`));
        process.exit(1);
    }
});
// optimize
cli
    ...
    .option('--force', `[boolean]  force the optimizer to ignore the cache and re-bundle`)
    .action(async (root, options) => {
    const { optimizeDeps } = await Promise.resolve().then(function () { return require('./chunks/dep-d7654bd1.js'); }).then(function (n) { return n.index; });
    try {
        const config = await build.resolveConfig({
            root,
            logLevel: options.logLevel
        }, 'build', 'development', options.config);
        await optimizeDeps(config, options.force, true);
    }
    catch (e) {
        const logError = build.createLogger(options.logLevel).error;
        logError(build.source.red(`error when optimizing deps:\n${e.stack}`));
        process.exit(1);
    }
});
cli.help();
